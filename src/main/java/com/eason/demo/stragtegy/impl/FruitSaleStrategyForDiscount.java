 ...
 */
@Service(Constants.SaleStrategyType.DISCOUNT)
@Slf4j
public class FruitSaleStrategyForDiscount implements FruitSaleStrategy {
    @Override
        ...
        for (int i = 0; i < fruitList.size(); i++) {
            ...
            try {
                if (fruit.getDiscount() > 0) {
                    totalPrice += ArithUtil.mul(fruit.getPrice(), fruit.getQuality(), fruit.getDiscount());
                } else {
                    totalPrice += ArithUtil.mul(fruit.getPrice(), fruit.getQuality());
                }
            } catch (Exception e) {
                log.error("【折扣策略计算总价，参数出现异常，参数为：{}】", fruit);
            }
        }
